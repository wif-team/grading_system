    ...
    void VerifyFaculty() {
        ...
        while (repetir == true) {
            do { // Ciclo para repetir en caso que no sea correcta la información
                ...
                tiene_numeros = false; // Inicializa el valor para comprobar nuevamente si la palabra que se introduce
                                       // tiene numeros
                char[] faculty_caracteres = new char[faculty.length()]; // Arreglo para tomar los caracteres
                                                                        // individuales de la palabra introducida
                ...
                if (faculty_caracteres[0] == ' ' || faculty_caracteres[faculty.length() - 1] == ' ') { // Ve si la
                                                                                                       // palabra inicia
                                                                                                       // o termina con
                                                                                                       // un espacio
                ...
                for (int i = 0; i < faculty.length(); i++) { // Ciclo para que revise letra por letra en busca de
                                                             // numeros
                    for (int j = 0; j < 10; j++) {
                        if (faculty_caracteres[i] == numeros_array_char[j]) { // Esta condicion esta hecha con arreglos
                                                                              // de char, al inicio es necesario
                            valid = false; // cambiar las arreglos de tipo int a char para que funcione esta condicion
                            tiene_numeros = true; // Boolean cambia de valor para que de el mensaje de que existe
                                                  // numeros
                            continue;
                        }
                    } // Condiciona que no haya más de dos espacios seguidos
                    if (faculty_caracteres[i] == 32 && faculty_caracteres[i + 1] == 32) { // "32" son espacios en codigo
                                                                                          // ascii
            ...
            opcion_repetir = JOptionPane.showConfirmDialog(null, "Desea ingresar nuevamente la facultad",
                    "Eliga una opcion", JOptionPane.YES_NO_OPTION);
